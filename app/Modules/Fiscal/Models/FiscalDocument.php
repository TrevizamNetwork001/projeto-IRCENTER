...
final class FiscalDocument extends Model
{
    ...
    protected $fillable = ['core_client_id', 'fiscal_issuer_profile_id', 'fiscal_customer_profile_id', 'billing_contract_id', 'invoice_id', 'charge_id', 'billing_item_id', 'environment', 'provider', 'status', 'competence_date', 'service_date', 'services_amount', 'discount_amount', 'deduction_amount', 'withholding_amount', 'net_amount', 'currency', 'summary', 'idempotency_key', 'external_reference', 'nfse_number', 'verification_code', 'authorized_at', 'cancelled_at', 'rejection_code', 'rejection_message'];

    protected function casts(): array
    {
        return ['core_client_id' => 'integer', 'environment' => FiscalEnvironment::class, 'status' => FiscalDocumentStatus::class, 'competence_date' => 'date', 'service_date' => 'date', 'services_amount' => 'decimal:2', 'discount_amount' => 'decimal:2', 'deduction_amount' => 'decimal:2', 'withholding_amount' => 'decimal:2', 'net_amount' => 'decimal:2', 'issuer_snapshot' => 'array', 'customer_snapshot' => 'array', 'service_snapshot' => 'array', 'values_snapshot' => 'array', 'tax_snapshot' => 'array', 'prepared_at' => 'immutable_datetime', 'authorized_at' => 'immutable_datetime', 'cancelled_at' => 'immutable_datetime'];
    }
    ...
    protected static function booted(): void
    {
        ...
        static::updating(function (self $document): void {
            foreach (['issuer_snapshot', 'customer_snapshot', 'service_snapshot', 'values_snapshot', 'tax_snapshot', 'prepared_at'] as $field) {
                if ($document->getOriginal('prepared_at') !== null && $document->isDirty($field)) {
