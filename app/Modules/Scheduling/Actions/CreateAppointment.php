...
use App\Modules\Scheduling\Services\{SchedulingAudit,SchedulingNotificationCenter,SlotGenerator};
...
final class CreateAppointment {
 public function __construct(private SlotGenerator $slots, private SchedulingAudit $audit, private SchedulingNotificationCenter $notificationCenter){}
 public function execute(EventType $eventType,array $data,?int $actor=null): array {
  ...
  $appointment=DB::transaction(function()use($eventType,$data,$actor,$cancel,$reschedule){
   ...
   $item=Appointment::query()->create(['event_type_id'=>$locked->id,'client_id'=>$data['client_id']??null,'scheduled_start_at'=>$start->utc(),'scheduled_end_at'=>$start->addMinutes($locked->duration_minutes)->utc(),'timezone'=>$data['timezone'],'status'=>'confirmed','guest_name'=>$data['guest_name'],'guest_email'=>mb_strtolower($data['guest_email']),'guest_phone'=>$data['guest_phone']??null,'guest_company'=>$data['guest_company']??null,'notes'=>$data['notes']??null,'cancellation_token_hash'=>hash('sha256',$cancel),'reschedule_token_hash'=>hash('sha256',$reschedule),'created_by'=>$actor]);
   ...
   $this->audit->record('appointment.created',$item->id,$actor,['public_id'=>$item->public_id]);
   $event=$item->events()->latest('id')->firstOrFail();
   $this->notificationCenter->publish($item,'created',$event->id);
   return $item;
  },3);
