...
@section('content')
    ...
    @if(config('finance_fiscal.fiscal.enabled', false))
        <section class="panel"><header class="panel-header"><div><span class="panel-eyebrow">Fiscal</span><h2>Cadastro e documentos fiscais</h2></div><div class="page-actions">@if(auth()->user()->isAdministrator())<a class="button button-primary" href="{{ route('fiscal.documents.create',['client_id'=>$client->id]) }}">Novo documento</a><a class="button button-secondary" href="{{ route('fiscal.dashboard',['search'=>$client->displayName()]) }}">Ver documentos</a><a class="button button-secondary" href="{{ route('clients.fiscal.edit', $client) }}">Editar dados fiscais</a>@endif</div></header><dl class="details-list"><div><dt>Cadastro fiscal</dt><dd><strong>{{ $fiscalProfile?->document && $fiscalProfile?->legal_name && $fiscalProfile?->municipality_code && $fiscalProfile?->state ? 'Completo' : 'Incompleto' }}</strong></dd></div><div><dt>Documentos</dt><dd>{{ $fiscalDocumentCount }}</dd></div><div><dt>Última NFS-e</dt><dd>{{ $lastAuthorizedFiscalDocument ? $lastAuthorizedFiscalDocument->authorized_at->format('d/m/Y').' · '.$lastAuthorizedFiscalDocument->nfse_number : 'Nenhuma emissão registrada' }}</dd></div></dl></section>
    @endif
